<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="font-bold text-xl text-gray-800 dark:text-gray-100 uppercase">Genres</h1>
                @if(auth()->check() && (auth()->user()->isAdmin() || auth()->user()->isEmployee()))
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Create, edit, and manage genres.</p>
                @else
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">View genres.</p>
                @endif
            </div>
