...
<body class="bg-gray-50 text-gray-800">
    ...
    <div class="max-w-3xl mx-auto py-8 px-4">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold">Track bewerken</h1>
            <p class="text-sm text-gray-500">Pas de gegevens van het nummer aan.</p>
        </div>

        <div class="bg-white shadow-sm rounded-lg p-6">
            <form action="{{ route('tracks.update', $track->id) }}" method="post" class="space-y-4">
                @csrf
                @method('PUT')

                <div>
                    <label for="title" class="block text-sm font-medium text-gray-700">Titel *</label>
                    <input type="text" name="title" id="title" required value="{{ $track->title }}" class="mt-1 block w-full border rounded px-3 py-2">
                </div>
                ...
                <div>
                    ...
                    <select name="album_id" id="album_id" required class="mt-1 block w-full border rounded px-3 py-2">
                        ...
                        @foreach($albums as $album)
                            <option value="{{ $album->id }}" {{ $track->album_id == $album->id ? 'selected' : '' }}>{{ $album->title }} - {{ $album->artist->name ?? '-' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="track_number" id="track_number" value="{{ $track->track_number }}" class="mt-1 block w-full border rounded px-3 py-2">
                    </div>

                    <div>
                        <label for="duration" class="block text-sm font-medium text-gray-700">Duur (seconden)</label>
                        <input type="number" name="duration" id="duration" value="{{ $track->duration }}" class="mt-1 block w-full border rounded px-3 py-2">
                    </div>
                </div>
